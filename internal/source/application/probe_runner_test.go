...
func TestProbeRunnerPropagatesRequestCancellation(t *testing.T) {
	...
	if domain.ProbeCancelled(ctx, errors.New("other")) {
		t.Fatal("unrelated error must not be classified as cancellation")
	}
	if !errors.Is(infrastructure.ProbeContext(ctx), context.Canceled) {
		t.Fatal("infrastructure must observe cancellation directly")
	}
	if !infrastructure.ProbeContextsIndependent(ctx, context.Background()) {
		t.Fatal("request contexts must remain independent")
	}
